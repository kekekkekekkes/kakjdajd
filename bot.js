...
client.on('message', message => {
    if (message.content === '-help') {
        ...
        .setTitle('**اوامر عامه اكتب help-all**')
        .setDescription('**اوامر اداره اكتب help-admin**')
        .addField('BY KaNe')
...
client.on('message',function(message) {
  ...
  const swearWords = ["يا منتاك","يا ابن المتناكه","يا ابن الشرموطه","تعالي انيكك","متناك","يا قحبه","يا عرص","كس امك","كسمك"];
  ...
  client.on('message',function(message) {
  if(!message.channel.guild) return undefined;
  const swearWords = ["استغفر الله"];
  if (swearWords.some(word => message.content.includes(word)) ) {
    message.delete()
    message.reply("اللهم قوي ايمانك"); 
  }
});

client.on('message',function(message) {
  if(!message.channel.guild) return undefined;
  const swearWords = ["هلا ","اهلا","."];
  if (swearWords.some(word => message.content.includes(word)) ) {
    message.delete()
...
client.on('message',function(message) {
  if(!message.channel.guild) return undefined;
  const swearWords = ["كيفك ","ايش مسوي","الاخبار"];
  if (swearWords.some(word => message.content.includes(word)) ) {
    message.delete()
    message.reply("تمام وينك يا سحبه"); 
  }
});
...
client.on('message',  message => {
  ...
  if(message.content.startsWith(prefix + "mute")) {
    ...
    mention.addRole(role).then(() => {  //by KaNe 
    ...
    let mention = message.mentions.members.first();
    let role = message.guild.roles.find('name', 'Muted') || message.guild.roles.get(r => r.name === 'Muted');
    if(!message.member.hasPermission("MANAGE_ROLES")) return message.reply('**أنت لا تملك الخصائص اللازمة . يجب توفر خاصية `Manage Roles`**').then(msg => { // BY KaNe
      msg.delete(3500);
      message.delete(3500);
    });
    ...
    if(!mention) return message.reply('**منشن الشخص لفك الميوت عنه**').then(msg => { 
      ...
      mention.removeRole(role);
      mention.setMute(false);
      message.channel.send(`**:white_check_mark: ${mention.user.username} unmuted in the server ! :neutral_face:  **  `); // BY KaNe
  }
});
...
.addField('    سادسا' ,' لا تنشر في الشات او بل خاص ')//KaNe
.addField('    سابعا' ,' لا تنشر روابط! ')
.addField('    ثامنا' ,' لا تسوي سبام ايموجي ')
.addField('    تاسعا' ,' لا تطلب رتبه الاداره ! ')
.addField('اي شخص بيخالف القوانين سيعرض للعواقب')
.setColor('#7d2dbe')
            .setFooter(message.author.username, message.author.avatarURL)

  message.channel.sendEmbed(embed);//KaNe
              ...
              m.channel.awaitMessages( m1 => m1.author == message.author,{ maxMatches: 1, time: 60*1000 } ).then ( (m1) => {
                  ...
                  m.edit(message.member + ', **:timer:**').then( (m) =>{
                      ...
                      m.edit(message.member + ', **....جارى جمع البيانات**').then( (mtime)=>{
                        ...
                        .addField('**`Name|الاسم`**', `${name}` , true)
                        .addField('**`IGN|اسمك باللعبه`**', `${name}` , true)
                        .addField('**`age|العمر`**', `${age}` , true)
                        .addField('**`Will it interact?|هل سيتفاعل  ؟`**',`${ask}`)
                        .addField('**`Will You respect the laws?|هل سيحترم القوانين ؟`**',`${ask2}`)
                        .addField('**`Why should we accept it?|لماذا يجب علينا قبوله ؟`**',`${ask3}`)
  ...
  client.on('message', msg => {
    ...
    message.channel.send({
        embed: new Discord.RichEmbed()
            ...
            .addField('**Bot Owner**👑 :' , `[<@390412085378547714>]` , true)
            .setFooter(message.author.username, message.author.avatarURL)
    })
}
});
...
if(!message.author.id === '390412085378547714') return;
...
client.on('ready', () => {
  ...
  client.user.setGame(`BY KaNe`,"http://twitch.tv/y04zgamer")
   client.user.setStatus("dnd")
});
...
client.on('ready', function(){
    var ms = 10000 ;
    var setGame = [`『TK』I Clan | Fortnite |`,`By : KaNe `];
    var i = -1;
    var j = 0;
